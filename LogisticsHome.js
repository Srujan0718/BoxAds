            ...
            <div className="dashboard-container-default">
                ...
                <div className="dashboard-column">
                    ...
                    <div className="mini-container mini-associations">
                        <div className="mini-container-head">Transactions</div>
                        <a href="#"><div className="show-more">+ <span>Show More</span></div></a>
                        <div className="logistic-transaction-details">
                            {paymentsStatus === "on" ? 
                                <>
                                    <div className="payout-threshold">
                                        <div className="progress-status">{printData.total_prints} * {printData.print_cost} / {printData.payment_threshold} </div>
                                        <div className="progress-bar" style={{ backgroundSize: `${progressPayment}%` }}></div>
                                        <style>
                                            {`
                                                @keyframes p1{
                                                    0% { background-size: 0%; }
                                                    100% { background-size: ${progressPayment}%; }
                                                }
                                                `}
                                        </style>
                                    </div>
                                    {progressPayment < 100 ? (
                                        <div className="request-payout" style={{ backgroundColor: "#aaa" }}>Request Payout</div>
                                    ):  <div className="request-payout" onClick={handleRequestPayout}>Request Payout</div>}
                                </>
                            : <>Payments are switched off by the Admin. Contact <a href='mailto:admin@boxads'>admin@boxads</a></>}
                        </div>
                    </div>
                </div>
            </div>
